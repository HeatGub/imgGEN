...
def scan_paths_and_index_images(root_dir, exclude = None):
    try:
        ...
        exclude = set(exclude or []) # not even used for now
        images_found_list = []
        ...
        image_paths = (
            ...
            and item.parent.name != "converted"
            # and not any(part in exclude for part in item.parts)  # exclude if any part of path matches exclude
        )
        ...
        database.select_all()
    except Exception as e: # does not catch system-exiting exceptions
        logger.error(f"Error during image paths indexing: {e}", exc_info=True)
